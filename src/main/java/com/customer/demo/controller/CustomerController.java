...
public class CustomerController {
	...
	public ResponseEntity<Map<String, Object>> create(@Valid @RequestBody CustomerRequest customerRequest) {
		LOGGER.info("Request for create received with {}", customerRequest.toString());
	...
	public ResponseEntity<Map<String, Object>> getById(
		...
		LOGGER.info("Request for get by id received with ID: {}", id);
	...
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "404", description = "Customer not found", content = @Content) })
	@GetMapping
	public ResponseEntity<Map<String, Object>> getByName(@RequestParam(required = false) String name,
			@RequestParam(required = false) String email) {
		LOGGER.info("Request for get by name or email received with name: {} email: {}", name, email);

		// If name is provided, fetch by name
		if (name != null) {
			return service.getByName(name).map(c -> ResponseEntity.ok(toResponse(c)))
					.orElse(ResponseEntity.notFound().build());
		}
		// If email is provided, fetch by email
		else if (email != null) {
			return service.getByEmail(email).map(c -> ResponseEntity.ok(toResponse(c)))
					.orElse(ResponseEntity.notFound().build());
		} else {
			return ResponseEntity.badRequest()
					.body(Collections.singletonMap("error", "Name or email must be provided"));
		}
	}
	...
	public ResponseEntity<Map<String, Object>> update(@PathVariable UUID id, @Valid @RequestBody Customer customer) {
		LOGGER.info("Request for update by id received with ID: {}", id);
	...
	public ResponseEntity<Void> delete(@PathVariable UUID id) {
		LOGGER.info("Request for delete by id received with ID: {}", id);
		service.delete(id);
